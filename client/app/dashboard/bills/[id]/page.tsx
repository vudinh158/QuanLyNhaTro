...
interface BillDetailsPageProps {
    params: { id: string }; // params không còn là Promise sau khi dùng React.use
}

export default function BillDetailsPage({ params: initialParams }: BillDetailsPageProps) {
    ...
    // Giữ nguyên cách truy cập params như Next.js Docs mới khuyến nghị
    // Thay đổi này là do cảnh báo ban đầu của bạn, và React.use() sẽ xử lý Promise
    // Tuy nhiên, khi truyền props trong Next.js App Router, `params` thường đã là một object
    // không phải Promise nữa. Nếu vẫn là Promise, `React.use` là cần thiết.
    // Trong trường hợp này, `useParams()` tự động giải nén.
    ...
    useEffect(() => {
        console.log("Invoice ID from params:", invoiceId); // DEBUG: Kiểm tra ID
        ...
        const fetchInvoiceDetails = async () => {
            setLoading(true);
            try {
                console.log(`Đang tải hóa đơn với ID: ${invoiceId}`); // DEBUG: Log trước khi gọi API
                const data = await getInvoiceById(invoiceId);
                console.log("Dữ liệu hóa đơn nhận được:", data); // DEBUG: Log dữ liệu từ API
                setInvoice(data);
            } catch (error: any) {
                console.error("Lỗi khi tải thông tin hóa đơn:", error); // DEBUG: Log lỗi chi tiết
                toast({
        ...
        fetchInvoiceDetails();
    }, [invoiceId, router, toast]); // Dependencies: invoiceId, router, toast
    ...
    // Render Skeleton nếu đang tải hoặc không có dữ liệu hóa đơn
    if (loading || !invoice) {
        return (
